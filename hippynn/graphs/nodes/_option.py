...
class _OptionModule(torch.nn.Module):
    def __init__(self, criteria_dict):
        ...
        self.moddict = torch.nn.ModuleDict(
            {self.criteria_names[fn]: mod for fn, mod in self.criteria_dict.items()}
        )
...
class OptionalMixin(ExpandParents,AutoKw):
    ...
    def _expand0(self,option,*,criteria_map,one_option):
        ...
        assert all(o.parents == option_parents for k,o in criteria_map),\
    ...
    def __init__(self, name, parents, criteria_map, module='auto',**kwargs):
        ...
        self.module_kwargs = {
            "criteria_map":{k:v.torch_module for k,v in criteria_map}
        }
...
class OptionalNode(OptionalMixin, SingleNode):
    def __init__(self,name,parents,criteria_map,module='auto',**kwargs):
        assert all(isinstance(v, SingleNode) for k, v in criteria_map),\
            "OptionalNode option types must be all BaseNode"
        one_option = one_value_from_dict(criteria_map)

        idxstate = one_option._index_state
        assert all(o._index_state == idxstate for o in criteria_map.values()),\
...
class OptionalMultiNode(OptionalMixin,MultiNode):
    def __init__(self, name, parents, criteria_map, module='auto', **kwargs):
        ...
        assert all(o._output_names == onames for o in criteria_map.values()), \
        ...
        assert all(o._output_index_states == child_states for o in criteria_map.values()), \
            "Sub-options must provide the same index structure to be compatible"
