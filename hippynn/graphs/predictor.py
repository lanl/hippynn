...
class Predictor():
    ...
    def __init__(self,inputs,outputs,return_device=torch.device('cpu'),model_device=None,requires_grad=False):
        ...
        outputs = [search_by_name(inputs,o) if isinstance(o,str) else o
                   for o in outputs]
    ...
    def add_output(self,node):
        ...
        if node not in self.graph.forward_output_list:
            raise ValueError("Node {} is not computed in graph. We suggest you make a ".format(node) +
    ...
    def __call__(self,*,node_values=None,single_prediction=False,batch_size=None,**kwargs):
        ...
        if len(node_values) != len(self.inputs):
            raise ValueError("Wrong number of node inputs. {} distinct inputs supplied, "
    ...
    def predict_batched(self,node_values,batch_size):
        ...
        full_outputs = {k:
                            torch.cat([batched_outputs[i][k] for i in range(n_batches)])
                        for k in out_keys}
        return full_outputs
