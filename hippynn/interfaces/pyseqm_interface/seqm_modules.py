...
def num_orb(species, target_method='wB97X'):
    ...
    if target_method=='wB97X':
        ...
        nvirt0 = torch.sum(species>0,dim=1)*4 - \
                 (torch.sum(species,dim=1)/2.0).type(torch.int64)
        return torch.min(norb, nocc+nvirt0), nocc
    elif target_method=='gaussian':
        nvirt0 = torch.sum(species>1,dim=1)*15 + torch.sum(species==1,dim=1)*2 - \
                 (torch.sum(species,dim=1)/2.0).type(torch.int64)
        return torch.min(norb, nocc+nvirt0), nocc
    else:
        raise KeyError("Computing orbital number \
...
class SEQM_OrbitalMask(torch.nn.Module):
    ...
    def forward(self, species):
        ...
        tmp = torch.arange(4*molsize, dtype=torch.int64, device=species.device) \
                   .reshape(1,-1).expand(nmol,4*molsize)
...
def pack_par(obj, species, par_atom, par_bond=None):
    ...
    if 'g_pp' in obj.learned and 'g_p2' in obj.learned:
        learned_parameters['g_pp'] = obj.softplus(learned_parameters['g_pp'] - learned_parameters['g_p2']) \
                                   + learned_parameters['g_p2'] + eps
...
class SEQM_All(torch.nn.Module):
    ...
    def forward(self, par_atom, coordinates, species):
        ...
        return Etot.reshape(-1,1), Etot_m_Eiso.reshape(-1,1), e, P, \
               Eelec.reshape(-1,1), Enuc.reshape(-1,1), Eiso.reshape(-1,1), charge, \
               notconverged.reshape(-1,1), atomic_charge
