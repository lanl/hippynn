...
class Hamiltonian_One(Hamiltonian):
    ...
    def forward(self, const, molsize, nHeavy, nHydro, nOccMO, Z, maskd, mask, atom_molid, pair_molid, idxi, idxj, ni,nj,xij,rij, parameters, P0):
        ...
        M, w = hcore(const, nmol, molsize, maskd, mask, idxi, idxj, ni,nj,xij,rij, Z, \
                     zetas,zetap, uss, upp , gss, gpp, gp2, hsp, beta)
...
class SEQM_One_Energy(SEQM_Energy):
    ...
    def forward(self, par_atom, coordinates, species, single_particle_density_matrix):
        ...
        Etot_m_Eiso, Etot = \
            self.energy(self.const, coordinates, species, \
                        learned_parameters=learned_parameters,
                        all_terms=True, P0=single_particle_density_matrix)[:2]
...
class SEQM_One_All(SEQM_All):
    ...
    def forward(self, par_atom, coordinates, species, single_particle_density_matrix):
        ...
        Etot_m_Eiso, Etot, Eelec, Enuc, Eiso, EnucAB, e, P, charge, notconverged = \
            self.energy(self.const, coordinates, species, \
                        learned_parameters=learned_parameters,
                        all_terms=True,P0=single_particle_density_matrix)
        ...
        return Etot.reshape(-1,1), Etot_m_Eiso.reshape(-1,1), e, \
               Eelec.reshape(-1,1), Enuc.reshape(-1,1), Eiso.reshape(-1,1), atomic_charge
...
class SEQM_One_EnergyNode(ExpandParents, AutoKw, MultiNode):
    ...
    def expand0(self, network, single_particle_density_matrix, seqm_parameters, \
        decay_factor = 1.0e-2, **kwargs):

        n_target_peratom = len(seqm_parameters["learned"])

        par_atom = HChargeNode("SEQM_Atom_Params",network,
            module_kwargs=dict(n_target=n_target_peratom,first_is_interacting=True))
...
class SEQM_One_AllNode(SEQM_One_EnergyNode):
    _input_names = "par_atom","Positions", "Species", "single_particle_density_matrix"
    _output_names = "mol_energy", "Etot_m_Eiso", "orbital_energies", \
                    "electric_energy", "nuclear_energy", "isolated_atom_energy", "atomic_charge"
    _main_output =  "Etot_m_Eiso"
    _output_index_states = (IdxType.Molecules,) * len(_output_names)
