...
class DirectoryDatabase(Database,Restartable):
    ...
    def __init__(self,directory,name,inputs,targets,*args,quiet=False,allow_unfound=False,**kwargs):
        ...
        self.restarter = self.make_restarter(directory,name,inputs,targets,
                                             *args,**kwargs,
                                             quiet=quiet,)
    ...
    def get_file_dict(self,directory,prefix):
        try:
            ...
            raise FileNotFoundError("ERROR: Couldn't find directory {} containing files."
                                    "A solution is to explicitly specify \"path\" in database_params ".format(directory)) from fee

        data_labels = {
            file[len(prefix):-4]:file
        ...
        if not data_labels:
            raise FileNotFoundError("No files found at {} .".format(directory) +
                             "for database prefix {}".format(prefix))
        return data_labels
...
class NPZDatabase(Database,Restartable):
    def __init__(self, file, inputs, targets, *args, allow_unfound=False,quiet=False, **kwargs):
        ...
        super().__init__(arr_dict, inputs, targets, *args, **kwargs, quiet=quiet)
        self.restarter = self.make_restarter(file, inputs, targets,
                                             *args, **kwargs,
                                             quiet=quiet,
