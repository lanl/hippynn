...
class HEnergy(torch.nn.Module):
    ...
    def forward(self, all_features, system_index, n_molecules):
        """
        ...
        partial_terms = [self.summer(x, system_index, n_molecules) for x in partial_energies]
        ...
        total_energies = self.summer(total_atomen, system_index, n_molecules)

        if self.n_terms > 1:
            partial_esq = [torch.square(x) for x in partial_energies]
            partial_atom_hier = [x / (x + y) for x, y in zip(partial_esq[1:], partial_esq[:-1])]
            mol_hier = [self.summer(x, system_index, n_molecules)/self.summer(x+y, system_index, n_molecules)
            ...
            total_hier = self.summer(total_atom_hier, system_index, n_molecules)
...
class LocalChargeEnergy(torch.nn.Module):
    ...
    def forward(self, charges, all_features, system_index, n_molecules):
        ...
        molecule_charge_energy = self.summer(atom_charge_energy, system_index, n_molecules)
...
class AtomizationEnergy(torch.nn.Module):
    ...
    def forward(self, all_features, vacuum_features, encoding, system_index, n_molecules):
