...
class Dipole(torch.nn.Module):
    ...
    def forward(self, charges: Tensor, positions: Tensor, system_index: Tensor, n_molecules: int):
        ...
        dipoles = self.summer(dipole_elements, system_index, n_molecules)
        return dipoles
...
class Quadrupole(torch.nn.Module):
    ...
    def forward(self, charges, positions, system_index, n_molecules):
        ...
        quadrupoles = self.summer(quad_elements, system_index, n_molecules)
        return quadrupoles
...
class CoulombEnergy(torch.nn.Module):
    ...
    def forward(self, charges, pair_dist, pair_first, pair_second, system_index, n_molecules):
        ...
        coulomb_molecule = self.summer(coulomb_atoms, system_index, n_molecules)
...
class CombineEnergy(torch.nn.Module):
    ...
    def forward(self, atom_energy_1, atom_energy_2, system_index, n_molecules):
        """
        ...
        mol_energy = self.summer(total_atom_energy, system_index, n_molecules)
