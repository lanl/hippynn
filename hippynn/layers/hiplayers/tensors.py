...
# even though it is about 20% faster at first, it is very bad overall.
# Compiling does not help.
def tk_calc_invariants(l_max:int, inv_list:List[int], tensor_features, C: List[Tensor]):
    intermediates: Dict[str, Tensor] = {}
    invariants: Dict[int, Tensor] = {}
    im = intermediates
    I = invariants
...
class TKHopInvariantLayer(torch.nn.Module):
    ...
    def __init__(self, inv_list, l_max, _cmaps=cmaps):
        super().__init__()
        self.l_max = l_max
        ...
        for n in inv_list:
            if n < 0 or n > 12:
                raise ValueError(f"Bad inv: {n}")

        if self.l_max < 0 or self.l_max > 3:
            raise ValueError(f"Bad l: {l_max}")

    def extra_repr(self):
        return f"inv_list={self.inv_list}, l_max={self.l_max}"
        ...
        if self.l_max > 0:
            ...
            if self.l_max > 1:
                ...
                if self.l_max > 2:
                    assert t is not None
        C = self.cmaps
        return tk_calc_invariants(self.l_max, self.inv_list, tensor_features, C)