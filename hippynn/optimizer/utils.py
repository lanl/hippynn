...
def debatch_numbers(numbers: torch.Tensor, padding_number: int=0, return_mask: bool=False) -> \
...
def debatch_coords(coords: torch.Tensor, masks: torch.Tensor) -> List[torch.Tensor]:
    ...
    # so this function must take masks as input, debatch to a list of tensors in diferent shape
...
def debatch(numbers: torch.Tensor, coords: torch.Tensor, numbers_padding: int=0, return_mask: bool=False) -> \
        Union[List[torch.Tensor], Tuple[List[torch.Tensor], torch.Tensor]]:
